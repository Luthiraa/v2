...
export default function BlogPost({ params }: { params: { slug: string } }) {
    ...
    if (!post) {
        return (
            <div className="min-h-screen bg-white text-black flex items-center justify-center p-4 text-center">
                <div>
                    <h1 className="text-2xl mb-4">404</h1>
                    <p className="text-gray-500 mb-8">Page not found</p>
                    <Link href="/writing"className="underline hover:text-gray-600">Return to writing</Link>
                </div>
            </div>
        );
    }
    ...
    return (
        <main className="min-h-screen w-full bg-white text-black p-6 md:p-12 selection:bg-white selection:text-black">
            {/* Header */}
            <header className="max-w-[650px] mx-auto w-full mb-16 flex items-center gap-4 text-xs text-gray-500">
                ...
                <Link href="/writing"className="hover:text-black transition-colors">
                    writing
                </Link>
                <span>/</span>
                <span className="text-gray-700 truncate max-w-[150px]">{post.slug}</span>
            </header>
            ...
            <article className="max-w-[650px] mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-700">
                <header className="mb-12 border-b border-black/10 pb-8">
                    ...
                    <h1 className="text-2xl md:text-5xl font-bold leading-tight tracking-tight">{post.title}</h1>
                </header>
            ...
            {/* Footer Navigation */}
            <div className="max-w-[650px] mx-auto w-full mt-24 pt-12 border-t border-black/10 flex justify-between text-xs text-gray-500">
                <Link href="/writing"className="hover:text-black transition-colors">← Back to writing</Link>
                <ScrollToTop />
            </div>
        </main>
    );
